...
close all
warning('off');
shapename='scene';%name of the stl file
...
%%
% 
filename="C:\Users\Arun Srivatsan\Google Drive\MIP\datasets\noiseFreeWithoutOutliers";
dlmwrite(char(strcat(filename,'\',shapename,'\F.txt')),F);
...
dlmwrite(strcat(filename,'\',shapename,'\C.txt'),SigmaS);
dlmwrite(strcat(filename,'\',shapename,'\Cb.txt'),B);
dlmwrite(char(strcat(filename,'\',shapename,'\f.txt')),SigmaS);
dlmwrite(strcat(filename,'\',shapename,'\o.txt'),B);
